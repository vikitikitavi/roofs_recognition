...
DIRECTORY = 'G:\\SVN\\Vika\\tf\\roofs'
LEARN = 'G:\\SVN\\Vika\\tf\\roofs\\learn'
TEST = 'G:\\SVN\\Vika\\tf\\roofs\\test'
ROUND = '\\round'
FLAT = '\\flat'
TRIANGULAR = '\\triangular'
SLASH = '\\'
DASH = '_'
PNG = '.png'
IMAGE_SIZE = 28
...
def create_new_set(roofs_shape, index):
    ...
    #     # img.save(LEARN + SLASH + str(persent) + DASH + str(index) + PNG)
    #
    # for rest in range(persent + 1, len(files)):
    #     img = Image.open(directory + SLASH + files[rest]).convert('L')
    #     img = img.resize((IMAGE_SIZE, IMAGE_SIZE), Image.ANTIALIAS)
    #     # img.save(TEST + SLASH + str(rest) + DASH + str(index) + PNG)
    #     for ind in range(0, 4):
    #         img = img.rotate(angle * ind)
